...
public class ClasificacionDAO {
	...
	public List<Clasificacion> getAllClasificacion() { //retorna todos los registros
		List<Clasificacion> claf = new ArrayList<>();
		...
		if (connection != null) {
			...
			try {
				...
				while (results.next()) {
					...
					int idTipoFlujo = results.getInt(3);
					Clasificacion clasificacion = new Clasificacion(id, nombre, idTipoFlujo);

					claf.add(clasificacion);
				}
			} catch (SQLException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
